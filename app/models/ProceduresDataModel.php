  ...
  class ProceduresDataModel extends Base
  {
    ...
    public function procedimientos($nombre,$tiempo){
      ...
      $sql = "SELECT * FROM (SELECT A.Actividad,ifnull(A.Meta,B.Realizado)as 'Meta',B.Realizado as 'Realizado', CONCAT(ROUND((B.Realizado/A.Meta)*100,2),'%') as '% realización' from (select p.name as 'Actividad',sum(g.number)as 'Meta' from goals g inner join procedures p on p.id= g.procedure_id inner join specialties s on s.id = p.specialty_id where s.name LIKE '%".$nombre."%' and g.date between '".$fecha1."' and '".$fecha2."' group by p.id) as A inner join
        ...
        UNION ALL
        SELECT p.name as 'Actividad',sum(pd.number) as 'Meta', sum(pd.number) as 'Realizado', '100.00%' as '% realización' from procedures p left outer join procedure_data pd on p.id =pd.procedure_id left outer join goals g on pd.procedure_id = g.procedure_id and g.date between '".$fecha1."' and '".$fecha2."' inner join specialties s on s.id = p.specialty_id where s.name LIKE '%".$nombre."%' and pd.date between '".$fecha1."' and '".$fecha2."' and g.id is null group by p.name
        UNION ALL
        SELECT p.name as 'Actividad',ifnull(sum(g.number),0) as 'Meta',0 as 'Realizado','0.00%' as '% realización' from procedures p left outer join procedure_data pd on p.id =pd.procedure_id and pd.date between '".$fecha1."' and '".$fecha2."' left outer join goals g on p.id = g.procedure_id and g.date between '".$fecha1."' and '".$fecha2."' inner join specialties s on s.id = p.specialty_id where s.name LIKE '%".$nombre."%'  and pd.id is null group by p.name
